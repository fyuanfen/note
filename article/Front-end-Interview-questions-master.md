本文旨在加深对前端知识点的理解，资料来源于网络，由本人(博客：http://zyy1217.com) 收集整理。
...
8. mysql与 MongoDB的区别
9. [IE与W3C怎么阻止事件的冒泡](http://caibaojian.com/javascript-stoppropagation-preventdefault.html)
10. 实现两个大整数的相加
11. 实现一个可拖动的div(要考虑到浏览器兼容性)
12. 浏览器缓存的区别
13. 304与200读取缓存的区别
14. h5地理定位api
15. vue与angularjs中双向数据绑定是怎样实现的
16. js私有变量怎么实现
17. ws和wss的区别
18. https的原理，加密方式，怎么加密的
19. bfs实现前序遍历；
20. 算法题，给千万量级的珠子，共上百种颜色，围成一个圈，求连续的包含所有颜色的最短子串的长度，并分析时间复杂度；
...
情景二：若已过期，则向服务器发送请求，此时请求中会带上1中设置的文件修改时间，和`Etag`
...
margin和padding都可以使用百分比值的，但有一点可能和通常的想法不同，就是 margin-top | margin-bottom | padding-top | padding-bottom 的百分比值参照的**不是容器的高度，而是最近的块级父元素的宽度** 。
...
**如果two为目标元素，目标元素的事情按顺序执行，而其他元素根据W3C的标准执行，即先捕获后冒泡**
...
1. 第一步是通过访问的域名找出其IP地址。DNS查找过程如下：
...
2. IE下,event对象有x,y属性,但是没有pageX,pageY属性; Firefox下,event对象有pageX,pageY属性,但是没有x,y属性.
...
[JSON 和JSONP详解](http://www.zyy1217.com/2016/12/02/JSON%20%E5%92%8CJSONP%E8%AF%A6%E8%A7%A3/)

[AJAX实现跨域的方法](http://www.zyy1217.com/2016/12/12/AJAX%E5%AE%9E%E7%8E%B0%E8%B7%A8%E5%9F%9F%E7%9A%84%E6%96%B9%E6%B3%95/)

[JavaScript跨域总结与解决办法](http://www.zyy1217.com/2016/12/03/%E8%B7%A8%E5%9F%9F/)


### JSONP：

原理是：动态插入`script`标签，通过`script`标签传递查询callback参数并引入一个文件，文件载入成功后会将这个callback参数作为函数名来包裹住JSON数据，最后执行我们在url参数中指定的函数。


由于同源策略的限制，`XmlHttpRequest`只允许请求当前源（域名、协议、端口）的资源，为了实现跨域请求，可以通过`script`标签实现跨域请求，然后在服务端输出JSON数据并执行回调函数，从而解决了跨域的数据请求。


优点是兼容性好，简单易用，支持浏览器与服务器双向通信。缺点是**只支持GET请求**。
...
- CORS与JSONP的使用目的相同，它是html5的新特性，比JSONP更强大。
- JSONP**只支持GET请求**，承载的信息量有限，CORS支持所有类型的HTTP请求，所以信息量较大时CORS是不二选择。
...
![](http://images.zyy1217.com/1490524322.png)
...
![](http://images.zyy1217.com/1490524690.png)
...
```
具体可参考：**[AJAX实现跨域的方法](http://www.zyy1217.com/2016/12/12/AJAX%E5%AE%9E%E7%8E%B0%E8%B7%A8%E5%9F%9F%E7%9A%84%E6%96%B9%E6%B3%95/)**
...
HTTP工作在应用层， HTTPS工作在传输层

默认HTTP的端口号为80，应用层`HTTPS`的端口号为443。
...
可参考[前端模块化](http://www.zyy1217.com/2016/12/30/%E5%89%8D%E7%AB%AF%E6%A8%A1%E5%9D%97%E5%8C%96/)
...
具体可参考[Javascript内存泄露以及循环引用解析](http://www.zyy1217.com/2016/12/10/Javascript%E5%86%85%E5%AD%98%E6%B3%84%E9%9C%B2%E4%BB%A5%E5%8F%8A%E5%BE%AA%E7%8E%AF%E5%BC%95%E7%94%A8%E8%A7%A3%E6%9E%90/)
